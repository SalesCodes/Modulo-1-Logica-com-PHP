...
O símbolo de (=) é justamente o carácter que faz a atribuição de um valor à uma variável, ou seja, na criação de uma variável como o exemplo abaixo:
...
Porém, também podemos executar operações dentro própria atribuição, ao colocarmos algum operador aritmético ou de concatenação anterior ao de atribuição, como mostra o exemplo abaixo:
